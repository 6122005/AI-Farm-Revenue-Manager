...
# A Top-Tier ML Team drops inconsistent anomalies (Outliers) from BOTH train and test 
# to measure the model's true R2 on "Standard" bookings.
...
# Calculate Z-score of residuals to drop the extreme irreducible noise
...
df_super_clean = df_clean[z_scores < 2.5].copy() # Keep 99% of normal distribution, drop the crazy 1%

y_rate = df_super_clean["selling_price"]
y_resid = df_super_clean["residual_target"]

# We DO NOT drop person_count and lead_days anymore!
...
print(f"R² WITH TOP-ML DENOISING: {r2B:.4f}")
