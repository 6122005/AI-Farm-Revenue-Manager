...
# Base mask (Festivals/Extended stay)
...
# Add interaction BEFORE baseline (so XGBoost gets it)
...
# SMART DENOISING: Only drop unexplainable noise. 
# Keep ALL Vacation and Festival spikes!
...
# Unexplainable noise = High Z-score BUT NOT a vacation AND NOT a festival
smart_noise_mask = (z_scores > 2.0) & (df_sorted["is_vacation"] == 0)
if "is_festival" in df_sorted.columns:
    smart_noise_mask = smart_noise_mask & (df_sorted["is_festival"] == 0)
...
print(f"R² (Smart Noise Drop + Vacation Rules preserved): {r2_score(y_test, preds_B):.4f}")
