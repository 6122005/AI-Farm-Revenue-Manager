...
# KEEP person_count and lead_days! So they are not in drop_cols.
...
# OUTLIER SMOOTHING TRICK
# Find the 95th percentile of absolute residuals in the training set and clip/remove them!
abs_resid = y_resid_train.abs()
p95 = abs_resid.quantile(0.95)
clean_mask = abs_resid <= p95

X_train_clean = X_train[clean_mask]
y_resid_train_clean = y_resid_train[clean_mask]

model_B = XGBRegressor(n_estimators=200, max_depth=5, learning_rate=0.05, random_state=42)
model_B.fit(X_train_clean[features], y_resid_train_clean)
...
print(f"R² WITH OUTLIER REMOVAL & FEATURES: {r2B:.4f}")
