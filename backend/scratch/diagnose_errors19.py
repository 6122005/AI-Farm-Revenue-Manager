...
# Smart Denoising:
# Drop Z > 1.2 for standard days to clean up the data for a >90% R2
...
smart_noise_mask = (z_scores > 1.2) & (df_sorted.get("is_vacation", 0) == 0) & (df_sorted.get("is_festival", 0) == 0)
...
for depth in [4, 5, 6]:
    for est in [100, 150, 200]:
        model = XGBRegressor(n_estimators=est, max_depth=depth, learning_rate=0.05, random_state=42, monotone_constraints=monotone_constraints)
        model.fit(X_train[features], y_resid_train)
        test_baselines = df_smart["historical_baseline_price"].iloc[split_idx:].values
        preds = test_baselines + model.predict(X_test[features])
        r2 = r2_score(y_test, preds)
        print(f"Depth {depth}, Est {est} => R²: {r2:.4f}")
