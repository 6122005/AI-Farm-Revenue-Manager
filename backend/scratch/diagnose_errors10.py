...
# DROP ALL HISTORICAL OUTLIERS!
if "historical_outlier_flag" in df_sorted.columns: drop_mask = drop_mask | (df_sorted["historical_outlier_flag"] == 1)
...
# DO NOT DROP person_count OR lead_days!
...
model_B = XGBRegressor(n_estimators=300, max_depth=6, learning_rate=0.03, random_state=42)
...
r2B = r2_score(y_test, preds_B)
print(f"R² (CLEANED + PERSON_COUNT + LEAD_DAYS): {r2B:.4f}")

# What if we drop person_count and lead_days just to see?
X_train_drop = X_train.drop(columns=["person_count", "lead_days"], errors='ignore')
X_test_drop = X_test.drop(columns=["person_count", "lead_days"], errors='ignore')
model_C = XGBRegressor(n_estimators=300, max_depth=6, learning_rate=0.03, random_state=42)
model_C.fit(X_train_drop, y_resid_train)
preds_C = test_baselines + model_C.predict(X_test_drop)
r2C = r2_score(y_test, preds_C)
print(f"R² (CLEANED NO PERSON_COUNT NO LEAD_DAYS): {r2C:.4f}")
